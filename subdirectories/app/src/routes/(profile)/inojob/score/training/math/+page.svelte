<script>
	import MathDetails from '$lib/components/inojob/training/math/MathDetails.svelte';
	import { fade, slide } from 'svelte/transition';
	import { afterUpdate, onMount } from 'svelte';
	import { getCookie, API_URL } from '$lib/main';
	import axios from 'axios';
	import MathAdd from '$lib/components/inojob/training/math/MathAdd.svelte';
	import Dropdown from '../../../../../../lib/components/Dropdown.svelte';
	$: updated = false;
	let showMathDetails = false;
	let showMathAdd = true;
	$: isPending = true;
	let error = false;
	let USER;
	$: USER;
...
>
	{#if !isPending}
		...
		{:else}
			<Dropdown>
				<div slot="header">مسابقه ریاضی دانشجویی کشور</div>
				<div slot="body">
					<MathDetails
						{showMathDetails}
						on:handleClick={() => {
