<script>
	import MedicalDetails from '$lib/components/inojob/training/medical/MedicalDetails.svelte';
	import { fade, slide } from 'svelte/transition';
	import { afterUpdate, onMount } from 'svelte';
	import { getCookie, API_URL } from '$lib/main';
	import axios from 'axios';
	import MedicalAdd from '$lib/components/inojob/training/medical/MedicalAdd.svelte';
	import Dropdown from '../../../../../../lib/components/Dropdown.svelte';
	$: updated = false;
	let showMedicalDetails = false;
	let showMedicalAdd = true;
	$: isPending = true;
	let error = false;
	let USER;
	$: USER;
...
>
	{#if !isPending}
		...
		{:else}
			<Dropdown>
				<div slot="header">آزمون های علوم پزشکی</div>
				<div slot="body">
					<MedicalDetails
						{showMedicalDetails}
						on:handleClick={() => {
