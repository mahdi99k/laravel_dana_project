<script>
	import BabaeiDetails from '$lib/components/inojob/skill/babaei/BabaeiDetails.svelte';
	import { fade, slide } from 'svelte/transition';
	import { afterUpdate, onMount } from 'svelte';
	import { getCookie, API_URL } from '$lib/main';
	import axios from 'axios';
	import BabaeiAdd from '$lib/components/inojob/skill/babaei/BabaeiAdd.svelte';
	import Dropdown from '../../../../../../lib/components/Dropdown.svelte';
	$: updated = false;
	let showBabaeiDetails = false;
	let showBabaeiAdd = true;
	$: isPending = true;
	let error = false;
	let USER;
	$: USER;
	...
	onMount(async () => {
		...
		await axios
			.get(`${API_URL}/getScores/skill/babaei`, {
...
>
	{#if !isPending}
		...
		{:else}
			<Dropdown>
				<div slot="header">شرکت و برگزیدگی در مسابقات مسئله محور(طرح شهید بابائی)</div>
				<div slot="body">
					<BabaeiDetails
						{showBabaeiDetails}
						on:handleClick={() => {
