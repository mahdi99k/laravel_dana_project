<script>
	...
	export let score;
	let degree = score.attributes.filter;
	let univetsities = [];
	let major = score.attributes.data.major,
		oriantation = score.attributes.data.oriantation,
		rank = score.attributes.data.rank,
		entry_half = score.attributes.data.entry_half,
		university = score.attributes.data.university,
		entry_year = score.attributes.data.entry_year,
		unit_numbers = score.attributes.data.unit_numbers,
		gpa = score.attributes.data.gpa,
		province = score.attributes.data.province,
		city = score.attributes.data.city,
		country = score.attributes.data.country,
		teacher = score.attributes.data.teacher,
		supervisor = score.attributes.data.supervisor,
		status = score.attributes.data.status,
		defense = score.attributes.data.defense,
		thesis = score.attributes.data.thesis,
		course_type = score.attributes.data.course_type;
	...
	const handleClickSave = async () => {
		...
		if (
			...
			country == '' ||
			!degree ||
			(country.includes('ایران') && (!province || !city))
		) {
...
>
	<div class="flex flex-wrap pb-20">
		<SearchSelect
			...
			data={degrees}
			multi={true}
			title={degrees2[score.attributes.filter]}
			search={false}
			all={false}
			responsive={true}
			label="مقطع"
		...
		{#if country.includes('ایران') || !country}
			...
			/>
		{:else}
			<Input label="نام دانشگاه" bind:val={university} />
		{/if}
		<Input bind:val={oriantation} label="گرایش" required={false} />
		...
		<SearchSelect
			...
			label="وضعیت تحصیل"
		/>
		{#if degree != 'associate' && degree != 'mullah2' && degree != 'mullah3' && degree != 'mullah4' && degree != ''}
